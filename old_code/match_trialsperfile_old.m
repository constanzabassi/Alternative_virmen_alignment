...
for t = 1:length(sound_condition_array(file).VR_sounds)
    possible_outcome{t,1} = [sound_condition_array(file).ITI_sounds(t,1)];
    possible_outcome{t,2} = [sound_condition_array(file).VR_sounds{t,5}];
...
%     trueConditions{i} = strjoin(cellstr(num2str(trueMatrix(i, 2))));
% end
for i = 1:sizeTrueMatrix(1)
...
for i = 1:size_estimatedMatrix(1)
    ...
    %estimatedConditions{i} = strjoin(cellstr(num2str(estimatedMatrix{i, 2})));
end
...
% mseValues = zeros(numSections, 1);
% 
% % Now compare trueConditions and estimatedConditions using MSE
% mseValues = zeros(numSections, 1);
% for i = 1:numSections
%     trueSection = trueConditions(i:i+sectionSize-1);
%     mseValues(i) = mean((cellfun(@(x) (ismember(x, estimatedConditions)), trueSection)).^2);
% end
% [minMSE, bestTrueSectionIndex] = min(mseValues);
% file_matching_trials(file,:) = [bestTrueSectionIndex, bestTrueSectionIndex + sectionSize - 1];
% fprintf('Best matching true section of trials: %d to %d\n', bestTrueSectionIndex, bestTrueSectionIndex + sectionSize - 1);
...
%%
% Assuming you have the trueConditions and estimatedConditions as cell arrays of strings
...
windowSize = size_estimatedMatrix(1); % Adjust the window size as needed
numSections = length(trueStr) - windowSize + 1;
bestDistance = Inf;
bestSectionStart = 1;
% Iterate through sections with a sliding window
for i = 1:numSections
    trueSection = trueStr(i:i+windowSize-1);
    % Calculate Levenshtein distance for the current window against the estimated section
    distance = levenshteinDistance(trueSection, estimatedStr);
...
% Identify the section bounds based on the best match
bestSectionEnd = bestSectionStart + windowSize - 1;
fprintf('Best matching section of trials: %d to %d\n', bestSectionStart, bestSectionEnd);
fprintf('Levenshtein distance: %d\n', bestDistance);
file_matching_trials(file,:) = [bestSectionStart, bestSectionEnd];
end
