function imaging = align_virmen_data(dff,deconv,virmen_aq,alignment_info,data,dataCell)
%z-score all dff (assumes neurons x time)
z_dff=zscore(dff,0,2);

imaging_spk = [];
imaging ={};
start_it_difference = 0;
...
for vr_trial = 1:length(dataCell.dataCell)-1%1:length(dataCell.dataCell)-1 % virmen trials within each acquisition (ex: VR_stim01)
    [~,start_it] = find((data.data(1,:) - dataCell.dataCell{1,vr_trial}.time.start)>0,1);   %%the first virmen iteration during the current trial based on the start time from virmen
    start_it_difference = [start_it_difference,(data.data(1,start_it+1)-data.data(1,start_it))*86400];
    [~,end_it] = find((data.data(1,:) - dataCell.dataCell{1,vr_trial}.time.stop)>0,1);   %%%the last virmen iteration during the current trial
    end_it = end_it -2;
    start_time_next_trial = dataCell.dataCell{1,vr_trial+1}.time.start;
    
    iti_start_it = end_it+1; % start of the ITI is one unit after the end of the trial
    [~,iti_end_it] = min(abs(data.data(1,:) - start_time_next_trial)); %ITI ends at the start of the next trial -1
   ...
   % for each trial making sure you are within bounds of this tseries folder's iteration numbers else go to the next folder!
    if start_it>= virmen_aq(file_ind).actual_it_values(1)&& end_it <  virmen_aq(file_ind).actual_it_values(end) 
        ...
        output_data.iteration_ids = virmen_aq(file_ind).actual_it_values; % it_ids from virmen iterations
          ...
          if  iti_end_it < output_data.iteration_ids(end) && output_data.iteration_times(find(output_data.iteration_ids==start_it)) >= output_data.frame_times(1) ...
                  && output_data.iteration_times(find(output_data.iteration_ids==end_it))<=alignment_info(file_ind).frame_times(end)
              ...
              %now that we know this is a good trial with imaging data we can get more info
                  %movement in virmen time using virmen iterations for that trial!
            ...
            imaging(vr_trial).frame_id_maze = frame_ids;
            ...
            imaging(vr_trial).frame_id_iti = frame_ids;
...
% % %             start_it_ind = find(output_data.iteration_ids==start_it);
% % %             end_it_ind = find(output_data.iteration_ids==end_it);
% % %             iti_end_it_ind = find(output_data.iteration_ids==iti_end_it);
            if isfield(dataCell.dataCell{1, vr_trial}.maze,'is_stim_trial')%stim_dataset == 1
                imaging(vr_trial).is_stim_trial = [dataCell.dataCell{1, vr_trial}.maze.is_stim_trial] ;
            end 
            ...
            for frame = 1:size(imaging(vr_trial).frame_id_iti,2)
                ind = find(imaging(vr_trial).frame_id_iti-min(imaging(vr_trial).frame_id_iti)+1==frame,1,'first');  %%%here finding the index of the virmen iteration that matches this frame id, then takes the value only at that first iteration that matches the imaging frame id. Could in the future decide to bin across all virmen iterations that occur during an imaging frame
                if isempty(ind)==0
                    ...
                    this_reward(frame) = movement_in_virmen_time(vr_trial).is_reward(ind);
            ...
            movement_in_imaging_time.y_position_iti = this_y_position;
            movement_in_imaging_time.x_position_iti = this_x_position;
            movement_in_imaging_time.y_velocity_iti = this_y_velocity;
            movement_in_imaging_time.x_velocity_iti = this_x_velocity;
            movement_in_imaging_time.view_angle_iti = this_view_angle;
            movement_in_imaging_time.is_reward_iti = this_reward;
            movement_in_imaging_time.in_ITI_iti = this_ITI;
