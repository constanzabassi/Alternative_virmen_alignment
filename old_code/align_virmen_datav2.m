...
dif = trial_its_difference (trial_its,data);
...
for vr_trial = 1:length(dataCell.dataCell)-1%1:length(dataCell.dataCell)-1 % virmen trials within each acquisition (ex: VR_stim01)
    ...
    start_it = trial_its.start_trial_its(vr_trial)+dif.start_trial_its(vr_trial); 
    end_it = trial_its.end_trial_its(vr_trial)+dif.end_trial_its(vr_trial);  
    iti_start_it = trial_its.start_iti_its(vr_trial)+dif.start_iti_its(vr_trial); 
    iti_end_it = trial_its.end_iti_its(vr_trial)+dif.end_iti_its(vr_trial); 
