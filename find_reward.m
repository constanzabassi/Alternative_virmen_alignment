...
for file = 1:length(virmen_it) 
    ...
    for trial = within_all_trials
        if trial_info(trial).correct == 1 %if mouse was rewarded
            ...
            reward_loc_pure(trial,2) = sound_condition_array(file).ITI_sounds(sound_trial,2)+30 - reward_loc_pure(trial,1)/1000*digidata_its(file).sync_sampling_rate; %distance in digidata time
